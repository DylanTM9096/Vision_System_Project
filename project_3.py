...
def calib_n_track(image_list, checkerboard_dim, square_size,
                  reference_image, real_width_mm, real_height_mm,
                  frame_interval=1, calibration_dir="calibration_images"):
    """
    Detects checkerboard corners, performs camera calibration, tracks a planar object using SIFT + solvePnP.

    Parameters:
        image_list: Array of images to process
        checkerboard_dim: Diementions of real-life checker board (inside points)
        square_size: Measurment of indavidual checkerboard square
        reference_image: Source image for sift to detect image in video
        real_width_mm: Width of object to track
        real_hight_mm: Width of object to track
        frame_interval: The interval at which frames are extracted. Default is 1 meaning every frame.
        calibration_dir: Directory that each frame and output video gets saved into 
    ...
    # Defining the world coordinates for 3D points
    objp_cb = np.zeros((1, checkerboard_dim[0]*checkerboard_dim[1], 3), np.float32)
    objp_cb[0,:,:2] = np.mgrid[0:checkerboard_dim[0], 0:checkerboard_dim[1]].T.reshape(-1, 2)
    objp_cb *= square_size
    ...
    dist = dist.reshape(-1, 1) if dist.ndim == 1 else dist #from old func
    ...
    if ret:
        ...
        for i in tqdm.tqdm(range(len(valid_frames)), desc="Processing Frames"):
            ...
            obj_success, chec_rvec, chec_tvec = cv2.solvePnP(objp_cb, imgpoints[i], mtx, dist) #find rotation and position matrix of checkerboard
            if not obj_success:
                continue

            # Convert rotation vector to matrix for coordinate math
            rmat, _ = cv2.Rodrigues(chec_rvec)

            # The position of the camera in world coordinates
            cam_pos = -np.matrix(rmat).T * np.matrix(chec_tvec)
            # print("Camera Position in World Coords:\n", cam_pos)
            world_coords.append(cam_pos)
            rvec_history.append(chec_rvec)
            tvec_history.append(chec_tvec)
            
            #Sift detector section
            gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
            ...
            #  Solve for position using solvePnP 
            obj_success, obj_rvec, obj_tvec = cv2.solvePnP(objp_obj, img_pts, mtx, dist)
            if not obj_success:
                continue
            ...
            if np.any(np.isnan(obj_tvec)) or np.any(np.isinf(obj_tvec)):
                continue
                
            
            R_cb, _ = cv2.Rodrigues(chec_rvec)
            T_cb = np.eye(4)
            T_cb[:3,:3] = R_cb
            T_cb[:3,3] = chec_tvec.flatten()

            R_obj, _ = cv2.Rodrigues(obj_rvec)
            T_obj = np.eye(4)
            T_obj[:3,:3] = R_obj
            T_obj[:3,3] = obj_tvec.flatten()
            
            T_cb_inv = np.linalg.inv(T_cb)
            T_world_obj = T_cb_inv @ T_obj
            ...
            # Draw axes using solvePnP pose
            img = draw_axes_on_frame(img, mtx, dist, chec_rvec, chec_tvec, imgpoints[i], checkerboard_dim, square_size)
            
            line_3d = np.float32([
                [0, 0, 0],   # checkerboard origin
